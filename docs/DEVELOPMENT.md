...
현재 프로그램 버전은 `0.0.01`, 사용자 표시 버전은 `v0.0.01`입니다. 초기 개발 단계에서는 `v0.0.01`, `v0.0.02` … `v0.0.10`처럼 증가시키고, 큰 기능 단계가 완성되면 `v0.1.0`, `v0.2.0`처럼 올릴 수 있습니다.
...
- `ProjectInfoDialog`: 현재 프로젝트 모델의 읽기 전용 스냅샷을 표시합니다.
...
`File > Project Info…`는 대화상자를 열 때마다 현재 `Project`, 저장 경로, dirty 상태를 읽습니다. 이 창은 편집 기능을 제공하지 않으며 프로그램 버전과 프로젝트 `format_version`을 별도 항목으로 표시합니다.
